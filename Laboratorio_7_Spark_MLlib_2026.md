...
**Fuente:** [Encuesta Nacional de Empleo e Ingresos, INE Guatemala](https://www.ine.gob.gt/encuesta-nacional-de-empleo-e-ingresos/)
...
Las categorías de `P05C16` incluidas son:
...
$$
\text{Antigüedad (en años)} = \text{antiguedad\_anios} + \frac{\text{antiguedad\_meses}}{12}
$$
...
## Análisis exploratorio avanzado y segmentación (25 puntos)
...
Construya una segmentación (*clustering*) usando las variables que considere adecuadas; analice si vale la pena incluir salario. Estandarice las variables y determine un número adecuado de $K$ *clusters* (2, 3, 4 y 5). Elija el mejor $K$ según el criterio que tomen como grupo y asigne a cada *cluster* una descripción basada en sus resultados.

## Modelado supervisado para predicción (75 puntos)
...
1. `StringIndexer` y `OneHotEncoder` para las variables categóricas: nivel educativo, categoría ocupacional y dominio.
2. `VectorAssembler` para combinar las variables numéricas -edad, antigüedad y horas semanales- con las variables categóricas codificadas.
3. Estandarización de los predictores, mediante la opción interna de `LinearRegression` o mediante `StandardScaler`. No es necesario aplicar ambos mecanismos.
4. Un modelo `LinearRegression` para estimar el salario mensual.
...
1. `StringIndexer` y `OneHotEncoder` para las variables categóricas: nivel educativo, categoría ocupacional y dominio.
2. `VectorAssembler` para combinar las variables numéricas -edad, antigüedad y horas semanales- con las variables categóricas codificadas.
3. Un modelo `RandomForestRegressor` para estimar el salario mensual. Este modelo no requiere estandarización de los predictores.
...
- Enlace del repositorio usado para versionar el código.

> **Nota:** Se deberá evidenciar el trabajo colaborativo de todos los integrantes para optar por calificación.

## Fechas de entrega

- **Avance:** jueves 24 de septiembre de 2026 a las 17:20 h. Actividades correspondientes al análisis exploratorio y segmentación.
- **Entrega final completa:** domingo 27 de septiembre de 2026 a las 23:59 h.
